...
let InquiriesService = class InquiriesService {
    ...
    async create(createInquiryDto, userId) {
    ...
    async update(id, updateInquiryDto, requesterId, requesterRole) {
        const inquiry = await this.prisma.inquiry.findUnique({ where: { id } });
        if (!inquiry)
            throw new common_1.NotFoundException('Inquiry not found');
        if (requesterRole !== client_1.Role.ADMIN && inquiry.userId !== requesterId) {
